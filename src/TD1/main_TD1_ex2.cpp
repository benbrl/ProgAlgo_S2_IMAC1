...
float from_non_npi_to_npi(std::vector<std::string> const &tokens)
{
  std::stack<Token> stack;
...
int main()
{
  ...
  std::string expression_non_npi;
  std::cout << "votre expression :" << std::endl;
  std::getline(std::cin, expression_non_npi);

  std::vector<std::string> from_non_npi_to_npi(expression_non_npi);
